...
int SolveTriangularMatrixEquation(int m, int n, double **a, int read_row_offset, int read_col_offset,
                                  int write_row_offset, int write_col_offset);
...
int RecursiveCholeskyFactorization(int n, double **a, int a_row_offset, int a_col_offset)
{
    ...
    */
    ...
    else // use recursive code
    {
        ...
        error = SolveTriangularMatrixEquation(n2, n1, a, a_row_offset, a_col_offset, n1 + a_row_offset,
                                              a_col_offset);
        if (error != 0)
        {
            return error;
    ...
    for (k = 0; k < n; k++)
    {
        ...
        for (j = k + 1; j < n; j++)
        {
            if (a[j + read_row_offset][k + read_col_offset] != 0.0)
            {
                ...
                for (i = 0; i < m; i++)
                {
                    a[i + write_row_offset][j + write_col_offset] -= temp * a[i + write_row_offset][k +
                                                                                                    write_col_offset];
                } // end of i loop
            }
        } // end of j loop
